<p align="center">
<a href="https://github.com/nourkagha/dotfiles"><img width="265" height="177" src="https://i.imgur.com/dF2JK4o.png">
</p>

<p align="center">
<a href="https://pop.system76.com"><img src="https://img.shields.io/badge/OS-Pop!__OS_20.04_LTS-%2306989A?logo=data:image/png;base64,&style=for-the-badge" alt="OS"></a>
<a href="https://gnome.org"><img src="https://img.shields.io/badge/DE-GNOME-%2306989A?style=for-the-badge" alt="DE"></a>
<a href="https://zsh.org"><img src="https://img.shields.io/badge/shell-zsh-%2306989A?style=for-the-badge" alt="Shell"></a>
<a href="https://github.com/nourkagha/dotfiles"><img src="https://img.shields.io/github/repo-size/nourkagha/dotfiles?label=size&logo=&style=for-the-badge" alt="Size"></a>
<a href="https://github.com/nourkagha/dotfiles/commits"><img src="https://img.shields.io/github/last-commit/nourkagha/dotfiles?label=last%20update&style=for-the-badge" alt="Last update"></a>
</p>

![image](https://i.imgur.com/5XWFVcF.png)

## Installation

Clone the repository by passing `--recurse-submodules` to initialise and update all submodules and nested submodules, then run `install` in the `~/.dotfiles` directory:

```
git clone --recurse-submodules https://github.com/nourkagha/dotfiles .dotfiles && cd .dotfiles && ./install
```

## Booting

### UEFI

Install **rEFInd** as the boot manager.

```
sudo apt-add-repository ppa:rodsmith/refind
sudo apt update
sudo apt install refind
```

### Legacy BIOS

Install and run `os-prober` to check for other operating systems.

```
sudo apt install os-prober
sudo os-prober
```

Update GRUB.

```
sudo update-grub
```

Install **GRUB Customizer**.

```
sudo add-apt-repository ppa:danielrichter2007/grub-customizer
sudo apt update
sudo apt install grub-customizer
```

### Splash

[Pop!_OS plymouth theme](https://github.com/pop-os/plymouth-theme): Install the `pop-logo` theme for an animated Pop!_OS splash screen after booting.

```
sudo apt install plymouth-theme-pop-logo
sudo update-alternatives --config default.plymouth
```

Select the `pop-logo` theme instead of `pop-basic`.

```
sudo kernelstub -a splash
sudo kernelstub -v
sudo update-initramfs -u
```

Reboot to apply changes.

*Note that this theme may slightly increase boot times.*

## Desktop Environment

### GNOME Extensions

Install the [GNOME Shell Integration](https://addons.mozilla.org/en-US/firefox/addon/gnome-shell-integration/) browser extension.

- [Dash to Dock](https://extensions.gnome.org/extension/307/dash-to-dock/) - Fully customisable dock for favourite apps.
- [Dash to Panel](https://extensions.gnome.org/extension/1160/dash-to-panel/) - Similar to Dash to Dock, but with a Windows look.
- [GSConnect](https://extensions.gnome.org/extension/1319/gsconnect/) - Connects an Android phone using [KDE Connect](https://github.com/KDE/kdeconnect-android) to the desktop for phone notifications, file transfer and device control and sync options.
- [User Themes](https://extensions.gnome.org/extension/19/user-themes/) - Custom shell themes for desktop customisation. Theme folders must be placed in a created `~/.themes` directory and are applied and managed through GNOME Tweaks.
- [Sound Input & Output Device Chooser](https://extensions.gnome.org/extension/906/sound-output-device-chooser/) - Shows a list of output and input devices in the status menu.
- [Tray Icons: Reloaded](https://extensions.gnome.org/extension/2890/tray-icons-reloaded/) - Brings back tray icons (such as WINE apps) to the top panel.
- [Caffeine](https://extensions.gnome.org/extension/517/caffeine/) - Disable the screensaver and auto suspend.
- [GameMode](https://extensions.gnome.org/extension/1852/gamemode/) - GameMode status indicator.
- [Syncthing Indicator](https://extensions.gnome.org/extension/1070/syncthing-indicator/) - Syncthing status indicator.
- [Transmission Daemon Indicator](https://extensions.gnome.org/extension/365/transmission-daemon-indicator/) - Transmission status indicator. Transmission > Edit > Preferences > Remote > Allow remote access.

## Software

***For professional software (i.e. design and engineering), a list can be found on https://pop.system76.com.***

### Terminal

- [Alacritty](https://github.com/alacritty/alacritty) - Fast and customisable terminal. *Colours (normal and bright) are adjusted to be exactly like the default GNOME terminal on Pop!_OS, along with some opacity and default window size adjustments.*

### Gaming

#### General

- [Steam](https://store.steampowered.com/about) - All Steam games. Settings > Steam Play, enable all and use Proton.
- [Lutris](https://github.com/lutris/lutris) - All non-Steam games, such as Battle.net.
- [GameHub](https://github.com/tkashkin/GameHub) - Unified gaming library from different sources such as GOG and itch.io.
- [GameMode](https://github.com/FeralInteractive/gamemode) - Optimises the system and games for more performance during gaming.
- [Proton GE](https://github.com/GloriousEggroll/proton-ge-custom) - Custom version of Valve's Proton by GloriousEggroll.
- [MangoHud](https://github.com/flightlessmango/MangoHud) - Overlay for monitoring FPS, CPU, GPU, RAM, temperatures and more.

#### Games

- [Ajour](https://github.com/ajour/ajour) - Addon manager for World of Warcraft.
- [MultiMC](https://github.com/MultiMC/MultiMC5) - Minecraft launcher which allows launching and managing of multiple game instances each with their own mods and packs.
- [RuneLite](https://github.com/runelite/runelite) - Old School RuneScape client.

### Security

- [Bitwarden](https://github.com/bitwarden/desktop) - Online password manager.
- [KeePassXC](https://github.com/keepassxreboot/keepassxc) - Offline password manager.
- [LessPass](https://github.com/lesspass/lesspass) - Stateless password manager.
- [OTPClient](https://github.com/paolostivanin/OTPClient) - OTP authenticator. Supports vault imports from Aegis on Android.
- [Cryptomator](https://github.com/cryptomator/cryptomator) - Local and cloud file encryption.
- [VeraCrypt](https://github.com/veracrypt/VeraCrypt) - Disk encryption.
- [ProtonVPN](https://github.com/ProtonVPN/linux-cli-community) - Free VPN with paid plans.

### Media

- [GIMP](https://gitlab.gnome.org/GNOME/gimp) - Photo editing.
- [OpenShot](https://github.com/OpenShot/openshot-qt) - Video editing.
- [Kdenlive](https://github.com/KDE/kdenlive) - Video editing.
- [OBS Studio](https://github.com/obsproject/obs-studio) - Streaming/recording.
- [Flameshot](https://github.com/flameshot-org/flameshot) - Screenshots.
- [Peek](https://github.com/phw/peek) - GIF/screen recording.
- [VLC](https://github.com/videolan/vlc) - Video and audio player.
- [Spotify](https://www.spotify.com/us/download/linux) - Music streaming.
- [Lollypop](https://gitlab.gnome.org/World/lollypop) - Music player.
- [Audacity](https://github.com/audacity/audacity) - Audio editing.
- [Chatterino](https://github.com/Chatterino/chatterino2) - Twitch chat client.
- [Streamlink](https://github.com/streamlink/streamlink) - CLI tool to watch from various sources (like Twitch) in VLC. Also has a Twitch GUI client.
- [FreeTube](https://github.com/FreeTubeApp/FreeTube) - Private YouTube client.
- [Jellyfin](https://github.com/jellyfin/jellyfin) - Stream to any device from a self-hosted media server.
- [Foliate](https://github.com/jellyfin/jellyfin) - Ebook reader.

### Personal & Work

- [Thunderbird](https://www.thunderbird.net) - Mail.
- [Joplin](https://github.com/laurent22/joplin) - Notes.
- [Standard Notes](https://github.com/standardnotes/desktop) - Notes.

### Development

- [Visual Studio Code](https://github.com/microsoft/vscode) - Code editor.
- [VSCodium](https://github.com/VSCodium/vscodium) - Code editor.
- [Atom](https://github.com/atom/atom) - Code editor.
- [Vim](https://github.com/vim/vim) - Text editor.

### Social

- [Discord](https://discord.com) - Community chat platform.
- [Signal](https://github.com/signalapp/Signal-Desktop) - Private messenger.
- [Element](https://github.com/vector-im/element-desktop) - Chat client for Matrix.
- [Mattermost](https://github.com/mattermost/desktop) - Collaboration chat platform, used by System76 for Pop!_OS.
- [Fractal](https://gitlab.gnome.org/GNOME/fractal) - GNOME Matrix client (no encryption).
- [Giara](https://gitlab.gnome.org/World/giara) - Reddit client.
- [Cawbird](https://github.com/IBBoard/cawbird) - Twitter client.

### Files & Storage

- [Nextcloud](https://github.com/nextcloud/desktop) - Self-hosted cloud storage.
- [Syncthing](https://github.com/syncthing/syncthing) - File synchronisation between devices.
- [Rclone](https://github.com/rclone/rclone) - CLI/GUI tool based on rsync to upload, sync and encrypt files to any cloud provider.
- [Transmission](https://github.com/transmission/transmission) - Torrent client.

### Utilities

- [neofetch](https://github.com/dylanaraps/neofetch) - CLI tool to display system information.
- [OpenRGB](https://github.com/CalcProgrammer1/OpenRGB) - RGB control for all PC hardware.
- [Cadmus](https://github.com/josh-richardson/cadmus) - Real-time microphone noise suppression. Alternative to Krisp and RTX Voice.
- [NoiseTorch](https://github.com/lawl/NoiseTorch) - Real-time microphone noise suppression.
- [Pop!_Paste](https://paste.pop-planet.info/client) - Pastebin CLI tool for https://paste.pop-planet.info.
- [Dconf Editor](https://gitlab.gnome.org/GNOME/dconf-editor) - GUI editor for GNOME shell settings and configuration.
- [Timeshift](https://github.com/teejee2008/timeshift) - System backup and restore.
- [Remmina](https://gitlab.com/Remmina/Remmina) - Remote desktop client.
- [Gufw](https://github.com/costales/gufw) - GUI for managing ufw (Linux firewall).
- [Stacer](https://github.com/oguzhaninan/Stacer) - All-in-one GUI system monitor utility.
- [QDirStat](https://github.com/shundhammer/qdirstat) - GUI disk usage analyzer based on WinDirStat.
- [Conky](https://github.com/brndnmtthws/conky) - System monitor for X which displays information on the desktop.
- [htop](https://github.com/htop-dev/htop) - CLI interactive process viewer and system monitor.
- [ncdu](https://dev.yorhel.nl/ncdu) - CLI disk usage analyzer.
- [TLP](https://github.com/linrunner/TLP) - CLI utility to save laptop battery power.
- [VirtualBox](https://www.virtualbox.org) - Virtual machines.
- [appimaged](https://github.com/probonopd/go-appimage/blob/master/src/appimaged/README.md) - AppImage daemon which automatically integrates AppImages into the desktop environment (i.e. desktop files) if they're present in locations such as `~/Applications`, `~/Downloads` or `~/bin`.
- [AppImageLauncher](https://github.com/TheAssassin/AppImageLauncher) - AppImage integration with the application launcher which allows management, updates and removal of AppImage files. Alternative to appimaged.

## Configuration & Customisation

### Terminal

- [Oh My Zsh](https://github.com/ohmyzsh/ohmyzsh) - A framework for managing and customising the zsh configuration. Requires installing zsh instead of the bash shell.
- [powerlevel10k](https://github.com/romkatv/powerlevel10k) - Zsh theme.
- [Oh My Bash](https://github.com/ohmybash/oh-my-bash) - Bash shell version of 'Oh My Zsh'.
- [Bash-it](https://github.com/Bash-it/bash-it) - Another customisation framework for bash.

### Dotfiles

- [dotfiles](https://dotfiles.github.io) - Dotfile management, configuration and backup.

### Themes

- [Spicetify CLI](https://github.com/khanhas/spicetify-cli) - Spotify custom CSS and themes. [Pop Dark](https://github.com/morpheusthewhite/spicetify-themes/tree/master/Pop-Dark).
- [VS Code Pop Theme](https://github.com/ArtisanByteCrafter/VSCodePopTheme) - Pop!_OS themes for Visual Studio Code.
- [Papirus Icon Theme](https://github.com/PapirusDevelopmentTeam/papirus-icon-theme) - Material and flat design icons.

## Community & Resources

- [Pop!_OS](https://pop.system76.com)
- [Pop!_Docs](https://support.system76.com/#pop)
- [Pop!_Community](https://chat.pop-os.org)
- [Pop!_Planet](https://pop-planet.info)
- [Pop!_OS Reddit](https://reddit.com/r/pop_os)
- [Pop!_OS Matrix](https://matrix.to/#/#pop_os:matrix.org)
- [Pop!_OS GitHub](https://github.com/pop-os)

## Additional Tweaks

- Command prefix to run applications using dedicated GPU: `env DRI_PRIME=1`
- Color profile: Standard Space - sRGB
- [Lutris Docs](https://github.com/lutris/docs)
- [Chris Titus Tech: The Ultimate Linux Gaming Guide](https://christitus.com/ultimate-linux-gaming-guide)
